...
class HDF5Logger:
    ...
    - proprio_observation: Вектор состояния робота.
    - action: Вектор действий, предпринятых агентом.
    ...
    def log_step(self, observation: np.ndarray, action: np.ndarray, reward: float, step_num: int):
        ...
        Args:
            observation (np.ndarray): Вектор состояния (проприоцепция + цель).
            action (np.ndarray): Вектор действий, предпринятых агентом.
            reward (float): Награда за текущий шаг (обычно 0.0).
...
if __name__ == '__main__':
    ...
    num_steps = 10
    observation_dim = 13 # 7 суставов + 3 цель + 3 клешня
    action_dim = 7 # 7 суставов
